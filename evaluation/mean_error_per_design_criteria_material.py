...
if __name__ == '__main__':
        ...
        for experiment in ['zero-shot']:
            for design in ['kitchen utensil grip', 'spacecraft component', 'underwater component', 'safety helmet']:
                for criteria in ['lightweight', 'heat resistant', 'corrosion resistant', 'high strength']:
                    for material in ["steel", "aluminium", "titanium", "glass", "wood", "thermoplastic", "elastomer", "thermoset", "composite"]:
                        # load model data
                        ...
                        model_data_df['mean_distance'] = model_data_df.apply(
                            lambda row: calculate_mean_error(row, survey_responses_df), axis=1)
    ...
    results_df.to_csv('mean_error_per_design_criteria_material.csv', index=False)
